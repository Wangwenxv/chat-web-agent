...
export function parseWorkspaceZip(file: File): Promise<ImportedWorkspace> {
  return new Promise((resolve, reject) => {
    if (file.size > 20 * 1024 * 1024) {
      reject({ message: 'ZIP is too large', detail: 'Imports are limited to 20 MiB.' } satisfies ImportError)
      return
    }
    ...
    reader.onload = () => {
      try {
        ...
        else reject({ message: 'Invalid ZIP file', detail: cause instanceof Error ? cause.message : String(cause) } satisfies ImportError)
      }
    }
...
const ALLOWED_IMPORT_EXTENSIONS = new Set(['.html', '.htm', '.css', '.js', '.mjs', '.json', '.svg', '.md', '.txt', '.ts', '.tsx'])

function inspectZip(data: Uint8Array): ImportedWorkspace {
  const decoded = unzipSync(data, { filter: (item) => !item.name.startsWith('__MACOSX/') && !item.name.endsWith('/') })
  const entries = Object.entries(decoded)
  if (entries.length > 1000) throw { message: 'Too many files in ZIP', detail: 'The workspace limit is 1,000 files.' } satisfies ImportError
  ...
  if (manifest) {
    try {
      const parsed = JSON.parse(decoder.decode(manifest[1])) as { title?: unknown; entryPath?: unknown }
      if (typeof parsed.title === 'string' && parsed.title.trim()) title = parsed.title.trim().slice(0, 80)
  ...
  for (const [name, bytes] of entries) {
    ...
    if (!ALLOWED_IMPORT_EXTENSIONS.has(extension)) {
      throw { message: 'Unsupported file in ZIP', detail: `${name}: extension ${extension || '(none)'} is not allowed.` } satisfies ImportError
    }
    const content = decoder.decode(bytes)
    totalBytes += content.length
    if (content.length > 1_048_576) throw { message: 'File too large in ZIP', detail: `${name} exceeds the 1 MiB per-file limit.` } satisfies ImportError
    if (totalBytes > 10 * 1024 * 1024) throw { message: 'ZIP content is too large', detail: 'The workspace limit is 10 MiB of text content.' } satisfies ImportError
  ...
  if (!files.some(file => file.path === entryPath || /\.html$/i.test(file.path))) {
    throw { message: 'ZIP has no HTML entry', detail: 'The preview needs an index.html or another .html file.' } satisfies ImportError
  }
