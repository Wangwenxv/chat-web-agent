import type { AgentSettings, ConnectionTestResult, ModelChunk, ModelMessage, ModelResponse, ToolCallRequest } from '../types'
...
const FORBIDDEN_HEADERS = new Set([
  'accept', 'accept-charset', 'accept-encoding', 'accept-language', 'connection', 'content-length',
  'content-type', 'cookie', 'date', 'dnt', 'expect', 'host', 'keep-alive', 'origin', 'referer',
...
function buildRequestHeaders(settings: AgentSettings, options: { streaming: boolean }): Record<string, string> {
...
export async function fetchModelList(settings: AgentSettings, signal?: AbortSignal): Promise<string[]> {
  ...
  if (!response.ok) {
    const raw = await response.text().catch(() => '')
    throw new Error(readApiError(raw ? (() => { try { return JSON.parse(raw) } catch { return null } })() : null, response.status))
  }
...
async function parseSseStream(
  ...
  const parseLine = (line: string): void => {
    ...
    const chunk = json as { choices?: Array<{ delta?: { content?: unknown; tool_calls?: unknown[] }; finish_reason?: string }>; usage?: ModelResponse['usage']; error?: unknown }
    if (chunk.error) {
...
function mergeToolCallPart(toolCalls: Map<string, ToolCallRequest>, part: unknown): void {
  ...
  const item = part as { id?: unknown; index?: unknown; type?: unknown; function?: { name?: unknown; arguments?: unknown } }
  if (typeof item.index !== 'number' || !item.function) return
  const existing = toolCalls.get(String(item.index))
  if (!existing) {
    if (typeof item.id !== 'string' || item.type !== 'function' || typeof item.function.name !== 'string') return
  ...
  if (typeof item.function.name === 'string' && typeof existing.function.name === 'string' && item.function.name !== '') {
    existing.function.name = item.function.name
  }
  if (typeof item.function.arguments === 'string') existing.function.arguments += item.function.arguments
}
...
export async function testModelConnection(settings: AgentSettings): Promise<ConnectionTestResult> {
  ...
  if (!settings.model.trim()) throw new Error('Model name is empty')
  try {
    const response = await fetch(baseUrl + '/models', { method: 'GET', headers: buildRequestHeaders(settings, { streaming: false }) })
    if (response.ok) {
      ...
      return { ok: true, message: '连接成功：' + settings.apiBaseUrl.trim() + (found ? '' : '。注意：服务器未列出 "' + settings.model.trim() + '"。') }
    }
    const raw = await response.text().catch(() => '')
    return { ok: false, message: 'HTTP ' + response.status + (raw ? '：' + raw.slice(0, 240) : '') }
  } catch (error) {
    const name = error instanceof DOMException ? error.name : ''
    const hint = name === 'AbortError' ? '请求超时。' : '请求可能被 CORS 拦截，多数模型 API 不允许浏览器跨域直连。'
    return { ok: false, message: name === 'AbortError' ? hint : (error instanceof Error ? error.message : String(error)) + '。' + hint }
  }
}
