...
import { AlertTriangle, Check, CheckCircle2, FileText, GitCompare, LoaderCircle, Save } from 'lucide-react'
import type { PreviewDiagnostic, WorkspaceFile } from '../../types'
import { buildLineDiff } from '../../lib/diff'

export function SourcePanel({ file, files, onSelectFile, onSave }: {
  file?: WorkspaceFile
  files: WorkspaceFile[]
    ...
    <div className="inspector-content source-content">
      <div className="source-heading">
        <label className="source-file-picker">
          ...
          <select className="source-file-select" value={file?.path ?? ''} onChange={event => onSelectFile(event.target.value)} disabled={files.length === 0 || dirty} title={dirty ? '请先保存或撤销当前修改' : '选择源码文件'}>
        ...
        <div className="source-actions">
          ...
          <button className="source-save-button" disabled={!dirty || saveState === 'saving'} onClick={() => void save()} title="保存源码">
            {saveState === 'saving' ? <LoaderCircle className="spin" size={13} /> : saveState === 'saved' ? <Check size={13} /> : <Save size={13} />}
            <span>{saveState === 'saving' ? '保存中' : saveState === 'saved' ? '已保存' : '保存'}</span>
          </button>
        </div>
      </div>
      {file
        ? <textarea className="source-editor" value={draft} onChange={event => { setDraft(event.target.value); setSaveState('idle'); setSaveError('') }} onKeyDown={event => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') { event.preventDefault(); void save() } }} spellCheck={false} aria-label={`${file.path} 源码编辑器`} />
        : <div className="empty-inspector"><FileText size={22} /><strong>请选择一个文件</strong><span>源码会与虚拟工作台实时同步。</span></div>}
      {saveError && <div className="source-save-error"><AlertTriangle size={13} /><span>{saveError}</span></div>}
    </div>
  )
}

export function ProblemsPanel({ diagnostics }: { diagnostics: PreviewDiagnostic[] }) {
  return <div className="inspector-content problems-content">{diagnostics.length === 0
    ? <div className="empty-inspector"><CheckCircle2 size={22} /><strong>未检测到问题</strong><span>最近一次预览构建没有报错。</span></div>
    : diagnostics.map((item, index) => <div className={'problem-item ' + item.level} key={item.message + index}>{item.level === 'error' ? <AlertTriangle size={15} /> : <CircleIcon level={item.level} />}<div><strong>{item.message}</strong>{item.detail && <pre>{item.detail}</pre>}</div></div>)}</div>
}
...
export function DiffPanel({ file, revisions }: { file?: WorkspaceFile; revisions: Array<{ revision: number; content: string; createdAt: number }> }) {
  ...
  return <div className="inspector-content diff-content"><div className="diff-heading"><span>{file.path}</span><span>r{file.revision}</span></div>{previous
    ...
    : <div className="empty-inspector compact"><GitCompare size={20} /><span>该文件没有更早的版本。</span></div>}</div>
}
