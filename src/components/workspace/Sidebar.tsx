  ...
  return (
    <aside className="sidebar" style={mobileStyle}>
      <div className="sidebar-section workspace-section">
        <div className="section-label">
          ...
          <span className="sidebar-heading-actions">
            <button className="mini-button" title="新建工作区" onClick={props.onNewWorkspace}>
              <Plus size={14} />
            </button>
            <button
              className="sidebar-toggle sidebar-inline-toggle"
              title="折叠侧栏"
        ...
        <div className="workspace-picker" ref={pickerRef}>
          <button
            className={'workspace-select' + (workspaceMenuOpen ? ' is-open' : '')}
            aria-haspopup="listbox"
            aria-expanded={workspaceMenuOpen}
            onClick={props.onToggleWorkspaceMenu}
          >
            <Sparkles size={14} />
          ...
          {workspaceMenuOpen && (
            <div className="workspace-menu" role="listbox">
              {workspaces.map((item) => (
                <button
                  className={'workspace-option' + (item.id === workspace.id ? ' is-selected' : '')}
                  role="option"
                  aria-selected={item.id === workspace.id}
                  key={item.id}
                  onClick={() => props.onSelectWorkspace(item.id)}
                >
      ...
      <div className="sidebar-section session-section">
        <div className="section-label">
          <span>会话</span>
          <button className="mini-button" title="新建会话" onClick={props.onNewSession}>
            <Plus size={14} />
          </button>
        </div>
      ...
      <div className="sidebar-footer">
        <button className="sidebar-action" onClick={props.onNewWorkspace}>
          <FolderPlus size={15} />
          <span>新建工作区</span>
        </button>
        <label className="sidebar-action" title="导入工作区 ZIP">
          <FolderUp size={15} />
          <span>导入工作区</span>
          <input
        ...
        <button className="sidebar-action" onClick={props.onOpenSettings}>
          <Settings2 size={15} />
          <span>设置</span>
        </button>
      </div>
