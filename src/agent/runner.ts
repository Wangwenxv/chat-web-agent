...
export async function runUserTurn(options: RunTurnOptions): Promise<RunTurnResult> {
  const { repository, workspaceId, sessionId, settings, signal, onEvent } = options
  let steps = 0
  let toolCallCount = 0
  ...
  try {
    while (steps < settings.maxSteps) {
      ...
      const response = await requestModel(settings, modelMessages, toolDefinitions, signal)
      if (response.toolCalls.length > 0) {
        ...
        for (const call of response.toolCalls) {
          ...
          if (result.changedPath) await emit('workspace_mutation', { path: result.changedPath, tool: call.function.name })
        }
        continue
      }
      ...
      await emit('turn_end', { steps, toolCalls: toolCallCount, segmented: settings.segmentResponses, segments: check.segments })
