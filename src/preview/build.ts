...
export function buildPreview(
  ...
  if (!entry) {
    diagnostics.push({
      level: 'error',
      message: 'No HTML entry file found',
      detail: 'Create an index.html file in the virtual workspace.',
    })
  ...
  for (const file of files) {
    if (file.kind === 'typescript')
      diagnostics.push({
        level: 'warn',
        message: file.path + ' is stored but not compiled in the browser preview',
      })
  }
  ...
  html = html.replace(/<link\b[^>]*href\s*=\s*["']([^"']+)["'][^>]*>/gi, (full, href: string) => {
    ...
    if (css?.kind === 'css')
      return (
        '<style data-workspace-file="' +
        escapeAttribute(css.path) +
        '">\n' +
    ...
    if (/^https?:\/\//i.test(href)) {
      ...
      diagnostics.push({
        level: 'warn',
        message: 'External stylesheet removed from preview',
        detail: href,
      })
      return ''
    }
  ...
  html = html.replace(
    ...
    (full, _before: string, src: string) => {
      ...
      if (js?.kind === 'javascript')
        return (
          '<script data-workspace-file="' +
          escapeAttribute(js.path) +
          '">\n' +
      ...
      if (/^https?:\/\//i.test(src)) {
        ...
        diagnostics.push({
          level: 'warn',
          message: 'External script removed from preview',
          detail: src,
        })
        return ''
      }
      if (js?.kind === 'typescript') {
        diagnostics.push({
          level: 'warn',
          message: src + ' is TypeScript and cannot run without a browser compiler',
        })
        return ''
      }
      return full
  ...
  const storageShim = perms.allowSameOrigin
    ? ''
    : '<script data-storage-shim>' + escapeScript(STORAGE_SHIM_SOURCE) + '</script>'
  ...
  const referencedScripts = new Set(
    Array.from(html.matchAll(/data-workspace-file="([^"]+)"/g), (match) => match[1]),
  )
...
function emptyDocument(diagnostics: PreviewDiagnostic[]): string {
  const details = diagnostics
    ...
    .join('')
  return (
    '<!doctype html><html><head><meta charset="UTF-8"></head><body style="font-family:system-ui;padding:32px">' +
    details +
    '</body></html>'
  )
}
